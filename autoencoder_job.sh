#!/bin/bash
#SBATCH --job-name=train_autoencoder # name
#SBATCH --nodes=1                    # nodes
#SBATCH --ntasks-per-node=1          # crucial - only 1 task per dist per node!
#SBATCH --cpus-per-task=64           # number of cores per tasks
#SBATCH --gres=gpu:4                 # number of gpus
#SBATCH --time 16:00:00              # maximum execution time (HH:MM:SS)
#SBATCH --output=delta_logs/train_autoencoder.out           # output file name
...
CUDA_VISIBLE_DEVICES=0,1,2,3 srun python3 train.py
